...
function easy_announcements_main() {
	...
	if ( $easy_announcements_loop->have_posts() ) :
		while ( $easy_announcements_loop->have_posts() ) : $easy_announcements_loop->the_post();
			if ( !empty( get_field( 'announcement_placement' ) ) && !empty( get_field( 'announcement_attachment' ) ) ) {
				$easy_announcements_ids[] = get_the_ID();
				$placement = get_field( 'announcement_placement' ) ?? '';
				$attachment = get_field( 'announcement_attachment' ) ?? '';
				...
				if ( !empty( $this_announcement ) ) $easy_announcements[$placement][$attachment][get_the_ID()] = $this_announcement;
			}
		endwhile;
		wp_reset_postdata();
